...
## With which frequency was each taxa encountered?
...
eukaryotes |>
  ...
  theme_classic()
```
### Did taxonomical composition of rare taxa change over time?
...
prokaryotes |>
  ...
  ggplot(aes(x = rate, fill = order)) +
  geom_bar(aes(weight = reads), position = "fill", color = "black", linewidth = .2) +
  facet_grid(. ~ season, scale = "free_x", labeller = as_labeller(labels_season)) +
  scale_y_continuous(labels = scales::percent, expand = c(0, 0)) +
  scale_fill_brewer(palette = "Set3", name = NULL) +
  labs(y = "Prokaryotic relative abundance", x = "Rate (%)") +
  theme_classic() +
...
## Did we find bacteria from human origin?
