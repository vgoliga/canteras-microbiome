...
asvs <- 
  prokaryotes |> mutate(community = "Prokaryotic", .before = 1) |>
  bind_rows(eukaryotes |> mutate(community = "Eukaryotic", .before = 1)) |>
  left_join(samples, by = join_by(sample)) |> 
  count(community, asv, encounters, probability_pct, rarity, wt = reads, name = "reads")
```
